...
const ERROR_CONFIG: LogViewerConfig<ErrorEntry> = {
  title: "Error Log",
  titleIcon: "\uF06A",
  titleColor: "#FF0040",
  borderColor: "#8B5CF6",
  ...
  getDetailHeader: (entry) => ({
    ...
    iconColor: entry.kind === "tool-ok" ? "#2d5" : "#FF0040",
    label: entry.name,
...
export function ErrorLog({ visible, messages, onClose }: Props) {
  ...
  return <LogViewer visible={visible} onClose={onClose} entries={entries} config={ERROR_CONFIG} />;
}
