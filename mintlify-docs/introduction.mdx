...
| **Code intelligence** | 4-tier fallback: LSP, ts-morph, tree-sitter, regex. Dual LSP. 12 operations. 30 languages | LSP via plugin marketplace (rename via cclsp, no auto-install) | No agent LSP access | MCP-based LSP via plugins | Tree-sitter AST |
...
| **Providers** | 10 (LLM Gateway, Anthropic, OpenAI, Google, xAI, Ollama, +4) | Anthropic (API, Bedrock, Vertex) | Multi-model | OpenAI models | 100+ LLMs |
...
<Note>Competitor capabilities verified as of March 28, 2026.</Note>
...
<Card title="4-tier code intelligence" icon="brain">
  LSP, ts-morph, tree-sitter, regex fallback chain across 30 languages with dual LSP backend.
</Card>
